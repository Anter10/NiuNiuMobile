...
function ServerWS:UserCreateRoomMsgReq(RoomInfo_table)
	...
	phen.rule.param_g=tonumber(RoomInfo_table.pay)
...
--创建房间
function ServerWS:UserCreateRoomMsgReq1(RoomInfo_table)
    local openid = cc.UserDefault:getInstance():getStringForKey("openid")
	local mas=Message_config.TYPES.MSG_C_2_S_CREATE_ROOM_REQ
	local phen =self.protoPb_list[Message_config.NAMES[tonumber(mas)]]()
	phen.msg_ID=mas
	...
	phen.rule.roomID=1
	phen.rule.gameType=1
	phen.rule.param_a=tonumber(RoomInfo_table.param_a)
	phen.rule.param_b=tonumber(RoomInfo_table.param_b)
	phen.rule.param_c=tonumber(RoomInfo_table.param_c)
	phen.rule.param_d=tonumber(RoomInfo_table.param_d)
	phen.rule.param_e=tonumber(RoomInfo_table.param_e)
	phen.rule.param_f=tonumber(RoomInfo_table.param_f)
	phen.rule.param_g=tonumber(RoomInfo_table.param_g)
...
function ServerWS:UserCreateRoomMsgRes_callback()
	   	...
	   	print("创建的当前玩家的房间信息 = ",decodeMsgData(self.msg))
	   	if  self.msg.m_errorCode ~= 0 then
	   		...
	   		elseif self.msg.m_errorCode == 3 then
	   			Util:fun_Offlinepopwindow("钻石不足，请前往商城购买! ",cc.Director:getInstance():getRunningScene(),
                function()
                   -- 弹出商城
                   cc.Director:getInstance():getRunningScene():addChild(AllRequire.ui.ShopView.create(),3209)
                end
	   			)
	   		end
        ...
        if self.msg.roomInfo.param_g == 1 then
           NotificationCenter:Instance():PostNotification("UserCreateRoomMsgRes")
        else
           Tools.showError("成功创建代开房间")
           self:UserSelectRoomsMsgReq()
        end
...
function ServerWS:UserActionMsgRes_callback()
	   	-- dump(self.msg)
	   	dump(self.msg.m_errorCode)
	   	-- Tools.showError("点击准备了啊"..self.msg.actionType)
	   	if self.msg.m_errorCode~=0 then
	...
	for i=1,#self.msg.m_info do
		...
		if self.msg.m_info[i].infotype==2 then	
			if self.msg.m_info[i].pictureurl then
			   -- self.loadpicture = self.msg.m_info[i].pictureurl
			   -- LocalData:Instance():set_Active("activity.png")--活动
			   -- Server:Instance():request_pic(self.loadpicture,"zy",1, "activity.png")
			end
		end
...
function ServerWS:requrstActivityImage()
	if self.loadpicture then
	   -- Server:Instance():request_pic(self.loadpicture,"zy",1,"activity.")
	end
end

function ServerWS:getLoadpicture()
	return self.loadpicture
end

function ServerWS:setLoadpicture(lop)
    self.loadpicture = lop
end
...
-- 请求玩家房间信息
function ServerWS:UserSelectRoomsMsgReq()
   local mas  = Message_config.TYPES.REQUEST_PLAYERS_ROOMS

   print("masmasmasmas = ",mas, Message_config.NAMES[tonumber(mas)],self.protoPb_list[Message_config.NAMES[tonumber(mas)]])
   local phen = self.protoPb_list[Message_config.NAMES[tonumber(mas)]]()
   phen.msg_ID=mas
   phen.gameType = 1
...
--玩家操作请求返回
--actionType 房间内玩家操作类型 1准备 2 开始  3 抢庄  4下注  5配牛 6 解散 7 响应解散（同意或拒绝
function ServerWS:UserSelectRoomsMsgRes_callback()
	   	 -- dump(self.msg)
	   	 -- dump(json.decode(self.msg))
	   	 -- dump(self.msg[1])
	   	 --dump(self.msg)
         ...
         -- json.decode(self.msg)
	   	 ...
	   	 --dump(#self.msg)
	   	 local rooms = {}
	   	 local datas = decodeMsgData(self.msg)
	    

         if AllRequire.ui.createRoom.createRoom then
         	AllRequire.ui.createRoom.createRoom:flushHouseList(datas)
         end


	   	 print("当前玩家的信息  =  ", json.encode(datas),AllRequire.ui.createRoom.createRoom)
	   	-- dump(self.msg["all"])
