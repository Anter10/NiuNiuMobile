...
-- 显示等待状态
function GameScene:showWaitStatue()
    
end


function GameScene:showReady(ready)
    self.gameScene:getChildByName("n_star_node"):setVisible(ready)
    self.gameScene:getChildByName("n_star_node"):getChildByName("bt_zb"):setVisible(ready)
    self.gameScene:getChildByName("waite"):setVisible(not ready)
end
...
function GameScene:init()
     ...
     self.excessiveLayer=AllRequire.ui.excessiveLayer.create()
     self:addChild(self.excessiveLayer,100,100)  
     self.excessiveLayer=AllRequire.ui.excessiveLayer.create()
     self:addChild(self.excessiveLayer,100,100)  
    ...
    dump(data.param_e)
    self.house_type = data.param_g
    -- Tools.showError("当前的房间类型  =  "..data.param_g)
    if data then
    ...
    local Integraloptions=self.gameScene:getChildByName("Integraloptions")
      Integraloptions:loadTexture("niuniu_youxijiemian/"..  dizhu..  ".png")


    local Integraloptions1=self.gameScene:getChildByName("Integraloptions1")   
    if jifen == 1  then
      Integraloptions1:loadTexture("niuniu_youxijiemian/n_ing_yifenchang.png")
    elseif jifen == 2  then
      Integraloptions1:loadTexture("niuniu_youxijiemian/n_ing_wufenchang.png")
    else

      Integraloptions1:loadTexture("niuniu_youxijiemian/n_ing_shifenchang.png")
    end
    local Integraloptions2=self.gameScene:getChildByName("Integraloptions2")
    
    print("抢庄类型  = ", zuozhuang)
    if zuozhuang == 2  then
      Integraloptions2:loadTexture("niuniu_youxijiemian/n_ing_fansizhangqiangzhuang.png")
    elseif zuozhuang == 3  then
      Integraloptions2:loadTexture("niuniu_youxijiemian/n_ing_jingdianqiangzhuang.png")
    else
      Integraloptions2:loadTexture("niuniu_youxijiemian/n_ing_lunliuqiangzhuang.png")
    end

    local Integraloptions3=self.gameScene:getChildByName("Integraloptions3")
    if is_tuogaun == 1  then
      Integraloptions3:loadTexture("niuniu_gameover/n_ing_chaoshituoguan.png")
    else
      Integraloptions3:loadTexture("niuniu_gameover/n_ing_chaoshibutuoguan.png")
    end
    ...
    bt_yq_wx_2:setPositionX(display.cx)
    ...
    --退出房间
    local n_star_node=self.gameScene:getChildByName("bt_tc")
      n_star_node:addTouchEventListener((function(sender, eventType  )
         self:touch_btCallback(sender, eventType)
    ...
    --解散房间
    local rmoveroomButton = self.gameScene:getChildByName("rmoveroom")
    -- if self.house_type == 3 or self.house_type == 4 then
       rmoveroomButton:addTouchEventListener((function(sender, eventType  )
             self:touch_btCallback(sender, eventType)
       end))
    -- end
    ...
    --  操作
      ...
      operation_friend:addTouchEventListener(function(sender, eventType  )
                  ...
                  local content = "房主支付 "
                  if self.house_type == 1 or self.house_type == 4 then
                      content = "AA支付 "
                  end

                  Util:share(self.gameControl.room_id, content)
      ...
      if getYKLogin() or (not Tools.iscompanyipa and isIOS() and getYKLogin()) then
    ...
    print("当前的房间类型 = ",self.house_type)
    -- if self.house_type == 3 or self.house_type == 4 then
       print("当前的房间类型2121212 = ",self.house_type)
       -- operation_dissolution:setVisible(false)--解散房间
       -- operation:getChildByName("operation_dissolution1"):setVisible(false)--解散房间
    -- end
...
-- 退出按钮隐藏和显示
function GameScene:visibleExitButton(vis)
     self.gameScene:getChildByName("bt_tc"):setVisible(vis)
end

-- 解散按钮隐藏和显示
function GameScene:visibleRemoveRoomButton(vis)
   self.gameScene:getChildByName("rmoveroom"):setVisible(vis)
end

-- 设置按钮的解散房间隐藏
function GameScene:visbleEditRemoveRoomButton( vis )
   self.gameScene:getChildByName("operation"):getChildByName("operation_dissolution"):setVisible()
end
...
function setZTControl(ztjr)
   ZTControl.hasintogame = ztjr
   print("setZTControl = ",ztjr)
   printError("中途加入在哪儿设置的")
end
...
function GameScene:touch_btCallback( sender, eventType)
            ...
            elseif touch_name=="bt_tc" then --退出房间
                --房主开始游戏
                self:dismis_room(1,nil,self.gameControl.user_buf)
            elseif touch_name=="rmoveroom" then --解散房间
                self:dismis_room(1,nil,self.gameControl.user_buf,1)
...
function GameScene:card_touch()
      ...
      if self.gameControl.show_card and #self.gameControl.show_card > 0 then
...
function GameScene:dismis_room(ttype,username,user_buf, buttontype)
  -- local  imageview=ccui.ImageView:create() 
  --  self:addChild(imageview)
  --  imageview:setTouchEnabled(true)
    self.clickbuttontype = buttontype
    ...
    if  ttype==1 then
       ...
       if not LocalData:Instance():get_Room_Start() then
          if LocalData:Instance():get_is_myhouse() == 0 and buttontype then
    ...
    self:upd_play(ttype,user_buf,username)
...
function GameScene:dismis_touch(sender, eventType,ttype,bg,user_buf)
        if eventType ~= ccui.TouchEventType.ended then
            return
        end
        Util:all_layer_Sound("Audio_Button_Click")
        ...
        -- Tools.showError("点击的按钮名称 = "..touch_name)
      if touch_name=="dis_btn_1" then
        ...
        if not self.clickbuttontype then
           -- Tools.showError("退出房间啊啊啊啊啊222",4)
        end

        if ttype==1 then
            if not LocalData:Instance():get_Room_Start() then
                 
                if LocalData:Instance():get_is_myhouse() ~= 0 or not self.clickbuttontype then
                   --非玩家退出房间
                   -- Tools.showError("退出房间啊dddddddddddddddddddddddddddddddddddddddddddddd啊啊啊啊3333",6)
                   ServerWS:Instance():UserActionMsgReq(8)
                   return
                end
            end
            -- Tools.showError("退出房间啊啊啊啊啊4444",8)
...
function GameScene:game_over()
    ...
    game_over:addChild(require("src.app.layers.touchlayer").create(), -1)
      ...
      local data=allscore[i]
       if data then
          ...
          if  tonumber(data.score) < 0  then
            ...
            jie_di:getChildByName("result_bg"..i):getChildByName("biaoji"):loadTexture("fonts/n_ing_jianhao1.png")
            jie_di:getChildByName("result_bg"..i):loadTexture("fonts/n_frame_jianhaodi1.png")
          else
            ...
            jie_di:getChildByName("result_bg"..i):getChildByName("biaoji"):loadTexture("fonts/n_ing_jiahao1.png")
            jie_di:getChildByName("result_bg"..i):loadTexture("fonts/n_frame_jiahaodi1.png")
          end
    ...
    local jie_xiangqing=game_over:getChildByName("jie_xiangqing")--详情
      ...
      jie_share:addTouchEventListener((function(sender, eventType  )
        ...
        -- self:fun_loadbar()
        jie_share:setTouchEnabled(false)
        local function removejp()
            -- self.loadbar:removeFromParent(true)
            -- self.loadba = nil
            jie_share:setTouchEnabled(true)
            Util:share(nil)
        end
        jie_share:stopActionByTag(500062)
        local seq = cc.Sequence:create(cc.DelayTime:create(1.5),cc.CallFunc:create(removejp))
        seq:setTag(500062)
        jie_share:runAction(seq)
...
function GameScene:fun_loadbar( )
        if not self.loadbar then
            ...
            self.loadbar:addChild(require("src.app.layers.touchlayer").create(true), -1)
...
function GameScene:call_back_up_layer()
        ...
        if cc.Director:getInstance():getRunningScene() then
           cc.Director:getInstance():replaceScene(require("app.views.gameHallScene").new())
        end  
