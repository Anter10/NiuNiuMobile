...
function gameHallScene:ctor()
     ...
     printError("那儿进来的啊得到的")
     Server:Instance():HttpSendpost("http://admin.sharkpoker.cn/index.php/home/login/get_active?channel="..Tools.Channel().channel.."&game="..Tools.Channel().gamename.."&account_id="..msg.openID,{}, function(data) 
         print("活动图 = ", json.encode(data))
         ServerWS:Instance():setLoadpicture(data.active_url)
    end)
     LocalData:Instance():start_scheduler(true)--心跳消息
    ...
    local files = require "src.app.views.ufiles"
    print("filesfilesfilesfiles")
    if Util:isFileExist(cc.FileUtils:getInstance():getWritablePath().."/src/app/views/ufiles.lua") then
       print("not has file")
    else
       print("has file")
    end
    -- printError("进入游戏大厅了"+files.version)
    ...
    ServerWS:Instance():requrstActivityImage()
    self:init() 
...
function gameHallScene:fun_new_radio_act(_text)
            ...
            print("_text_text_text_text ",_text)
...
function gameHallScene:init()
    ...
    local n_icon=g_bg:getChildByName("n_bg"):getChildByName("n_icon")--用户协议
    if n_icon and Tools.Channel().id == Tools.allchannel[2] then
       local sprite = cc.Sprite:create(Tools.Channel().logo)
       local px, py = n_icon:getPosition()
       sprite:setPosition(cc.p(px, py))
       n_icon:getParent():addChild(sprite,1)
       n_icon:removeFromParent(true)
    end
    ...
    local Button_14=self.gameScene:getChildByName("Button_14")--创建房间
    Button_14:addTouchEventListener((function(sender, eventType)
         self:touch_btCallback(sender, eventType)
    end))
    ...
    local Button_24=self.gameScene:getChildByName("Button_24")--创建房间
    Button_24:addTouchEventListener((function(sender, eventType)
         self:touch_btCallback(sender, eventType)
    end))
    ...
    if isIOS() then 
           ...
           Button_14:setPositionX(960-400)
           Button_24:setPositionX(960+400)
    end
    ...
    if isIOS()  then
       ...
       self.gameScene:getChildByName("Button_34"):setVisible(false)
    ...
    Button_11:addTouchEventListener((function(sender, eventType  )
         ...
         if eventType ~= ccui.TouchEventType.ended then
                  ...
                  local activityLayer = require("app.layers.activityLayer")  --设置 
                   print("活22动33")
                  self:addChild(activityLayer.new(),400,100)
    end))
...
function gameHallScene:touch_btCallback( sender, eventType)
            ...
            if touch_name=="Button_1" or touch_name=="Button_14"  then
               local creatm= AllRequire.ui.createRoom.new()
               self:addChild(creatm,10)
                -- ServerWS:Instance():UserCreateRoomMsgReq()
            elseif touch_name=="Button_2" or touch_name=="Button_24"  then 
...
function gameHallScene:ref_diamond()
      ...
      print("当前的消息2 = ",json.encode(decodeMsgData(_msg)))
      local stringnumber = self.gameScene:getChildByName("n_zs"):getChildByName("n_g_num"):getString()
      if tonumber(stringnumber) and tonumber(stringnumber) < _msg.value and _msg.type == 0 then
         if serverpayorder then
            Tools.showError("充值成功, 请注意查收。", 5, 50)
         end
      end
      serverpayorder = nil
  ...
  ServerWS:Instance():open_update()
      ...
      NotificationCenter:Instance():AddObserver("UserCreateRoomMsgRes", self,
                       function()
                          ...
                          if cc.Director:getInstance():getRunningScene() then
                             cc.Director:getInstance():replaceScene(require("app.views.GameScene").new({type=type_,old_join=false}))
                          end 
	 ...
	 NotificationCenter:Instance():AddObserver("UserJoinRoomMsgRes", self,
                       function()
                          ...
                          if cc.Director:getInstance():getRunningScene() then
                             cc.Director:getInstance():replaceScene(require("app.views.GameScene").new({type=type_,old_join=true}))
                          end 
                       		-- local layer=
    ...
    NotificationCenter:Instance():AddObserver("Marquee", self, function(target, data) --下载图片
                            if self.t_title then
                                  if LocalData:Instance():get_Marquee() then
                                       print("LocMarqueeMarqueealData:Instance():get_Marquee_new() = ",LocalData:Instance():get_Marquee())
                                     self:fun_radio_act(LocalData:Instance():get_Marquee())
                                     self.gamescene_radio_bg:setVisible(true)
                                     self.gamescene_horn:setVisible(true)
    ...
    NotificationCenter:Instance():AddObserver("MarqueeJS", self, function(target, data) --下载图片
                                         ...
                                         if self.t_title1 then
                                              if LocalData:Instance():get_Marquee_new() then
                                                print("LocalData:Instance():get_Marquee_new() = ",LocalData:Instance():get_Marquee_new())
    ...
    NotificationCenter:Instance():AddObserver("HistoryResultMsgRes", self,
                       function()
                        print("章鱼互动战绩消息")
                        if self.mian_Record_layer then
