...
function checkLayer:fun_list( ... )
	...
	for i=1,#game_over_data.sinfo do
          ...
          if i<=20 then  --   只做到20啦  后期美术改
             xiang_num:loadTexture("niuniu_gameover/n_ing_"  ..   i   ..   ".png")
             if i > 10 then
                xiang_num:setContentSize(cc.size(81, 60))
             end
          end
          ...
          for j=1,#RoundData.scoreinfo do
                ...
                if data then
                    ...
                    fen2:setString(data.score)
                     if tonumber(data.score) < 0 then  --  负数
                        fen1:setVisible(true)
                        fen2:setVisible(false)
                        biaoji:loadTexture("niuniu_gameover/n_ing_jianhao.png")
                        result_bg:loadTexture("niuniu_tanchuang/n_frame_jianhaodi.png")
                     else  --  正数
                        fen1:setVisible(false)
                        fen2:setVisible(true)
                        biaoji:loadTexture("niuniu_gameover/n_ing_jiahao.png")
