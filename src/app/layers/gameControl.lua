...
ROOM_STATUS[3]="niuniu_youxijiemian/n_ing_dengdaiqiangzhuang.png"
ROOM_STATUS[4]="niuniu_youxijiemian/n_ing_dengdaixiazhu.png"
ROOM_STATUS[5]="niuniu_youxijiemian/n_ing_dengdaopeiniu.png"
...
function gameControl:function_Desk_show_Control(Status)
    if Desk_Status.Desk_In== Status then
        ...
        self.tip_status:loadTexture("niuniu_youxijiemian/n_ing_dengdaiqiangzhuang.png")
        
        self:button_control(1)
       for i=1,#self.user_buf do
            if self.user_buf[i] then
               self.user_buf[i].user_c:getChildByName("wait"):setVisible(false)
            end
        end
...
function gameControl:button_control(Status)
  print("代理类型 = ",self.delegate.type)
  printError("按钮状态   = "..Status)
  ...
  if Status == 1 then
    ...
    if self.delegate.type~=0 then
        ...
        if AllRequire.ui.GameScene.GameScene then
           AllRequire.ui.GameScene.GameScene:showReady(true)
           print("是否处于等待状态1222  = ",waitstatue)
        end
        -- self:visibleWxyq(true)
  ...
  if Status==2 then
    ...
    self.gameScene:getChildByName("n_star_node"):getChildByName("bt_star_game"):setVisible(false)
    -- if AllRequire.ui.GameScene.GameScene then
    --    AllRequire.ui.GameScene.GameScene:visibleExitButton(false)
    -- end
  ...
  if Status==4 then--下注
      print("当前玩家是否为庄 = ", LocalData:Instance():get_is_myhouse())
  ...
  if Status==6 then--算牛
      local msg = ServerWS:Instance().msg
      -- print("msgmsgmsg当前的socket数据信息 = ",json.encode(decodeMsgData(msg)))
      self:add_time_upd("N_USER_STATE_BET")
      
      print("算牛的时候的现实  = ",ZTControl.hasintogame)
      if ZTControl.hasintogame then
...
function gameControl:function_Desk_Z(Status)
    ...
    if Status and ZTControl.hasintogame then
...
function gameControl:openCard(cardBg,cardFg)
    ...
    self.gameScene:runAction(cc.Sequence:create(cc.DelayTime:create(time),cc.CallFunc:create(function()
                                   print("什么情况啊啊啊啊啊什么情况啊啊啊啊啊什么情况啊啊啊啊啊",cardFg:getScale())
...
function gameControl:show_other_user_card()
  local c_msg=LocalData:Instance():get_NotifyRoomUserCardMsg()
    if self.user_Control_pai and type(self.user_Control_pai) == "table" and #self.user_Control_pai == 0 then
       -- self:function_Desk_Start()
       self:function_Desk()
       self:back_online_card()
       -- table.insert(self.user_Control_pai,table_board)
       print("当前是什么意思撒啊 啊啊啊")
       -- self::function_action_sque1()
    end
    dump(self.user_buf)
    -- local count = #self.user_buf
    -- if not ZTControl.hasintogame then
    --    count = count - 1
    -- end

    print("#self.user_buf#self.user_buf = ",#self.user_buf,#self.user_Control_pai)
    -- if c_msg.userCard and #c_msg.userCard == 1 then
    --    return 
    -- end
    for i=1,#self.user_buf do
      for j=1,#c_msg.userCard do
          if self.user_buf[i].pos ==c_msg.userCard[j].pos then
              ...
              for h=1,#c_msg.userCard[j].cards do
                  ...
                  print("self.user_Control_paiself.user_Control_paiself.user_Control_pai = ",#self.user_Control_pai, dex,h )
                  local spr=self.user_Control_pai[dex][h]
                  local coler = c_msg.userCard[j].cards[h].coler
                  local num = c_msg.userCard[j].cards[h].number

                  if num > 9 then
                     local chars = string.char(55 + num)
                     num = chars
                  end
                  
                  local str_pic=string.format("res/cards/MDCards/card_%s%s_md.png",coler-1,num)
                  -- self.user_Control_pai[dex][h]:ignoreContentAdaptWithSize(true)
                  self.user_Control_pai[dex][h]:loadTexture(str_pic)
                  -- self.user_Control_pai[dex][h]:setScale(1.4)
...
function gameControl:new_self_CardMsg(pai,coler,num)
   print("自己的拍张信息111 ",coler,num)
    if num > 9 then
       local char = 55 + tonumber(num)
       print("自己的拍张信息22-----",coler,char)
       local chars = string.char(char)
       num = chars
    end
     print("自己的拍张信息33-----",coler,num)
    local str_pic=string.format("res/cards/MDCards/card_%s%s_md.png",coler-1,num)
...
function gameControl:self_CardMsg()
   ...
   print(ZTControl.hasintogame, "--执行翻牌动画-",roomInfo.state)
   ...
   if #self.show_card > 0 then
     ...
     -- ZTControl.hasintogame = true
   end
    print(num,"card_numcard_numcard_num = ",card_num)
   if card_num > 0 then
...
function gameControl:other_niu_num(pos,ttype)

  print("··other_niu_num牛几- ·",pos,ttype,self.user_buf[6].pos)
    if ttype==-1 then
      return
    end
    Util:all_layer_Sound(string.format("niu_%s_w",ttype))

    local str_pic=string.format("niuniu_youxijiemian/n_ing_niu%s.png",ttype)
...
function gameControl:user_state_upd(dex,c_msg)
      if c_msg.actionType==1 then --1准备
        ...
        self.user_Banker_buf[1][dex]:loadTexture("niuniu_youxijiemian/n_ing_zhunbei.png")
      ...
      if c_msg.actionType==3 then --抢庄
        ...
        if c_msg.robValue==2 then
          self.user_Banker_buf[1][dex]:loadTexture("niuniu_youxijiemian/n_icon_buqiang.png")
           return
        end
        self.user_Banker_buf[1][dex]:loadTexture("niuniu_youxijiemian/n_icon_qiangzhuang.png")
      end
      ...
      if c_msg.actionType==4 then --4下注
        ...
        self.user_Banker_buf[2][dex]:loadTexture("niuniu_youxijiemian/n_ing_xiazhubeishu"..c_msg.betValue..".png")
      end
end
...
function gameControl:self_niu_info()
  ...
  print("---self_niu_info--",msg.actionType,msg.betvalue)
   if msg.actionType==4 then
       ...
       self.user_Banker_buf[2][self_num]:loadTexture("niuniu_youxijiemian/n_ing_xiazhubeishu"..msg.betvalue..".png")
   end
    ...
    if msg.actionType==3 then 
      print("---进入了没-3",msg.actionType,msg.robValue)
       ...
       if msg.robValue==2 then
          self.user_Banker_buf[1][self_num]:loadTexture("niuniu_youxijiemian/n_icon_buqiang.png")
         return
       end
       self.user_Banker_buf[1][self_num]:loadTexture("niuniu_youxijiemian/n_icon_qiangzhuang.png")
   end

   if msg.actionType==1 then 
       self:button_control(2)
       self.user_Banker_buf[1][self_num]:setVisible(true)
       self.user_Banker_buf[1][self_num]:loadTexture("niuniu_youxijiemian/n_ing_zhunbei.png")
   end
...
function gameControl:ref_all_head(head,dex)  
   ...
   local head_pic=Util:sub_str(head)
   if Util:isFileExist(head_pic) then
      self.user_buf[dex].user_c:getChildByName("head_pic"):loadTexture(head_pic)
   else
...
function gameControl:user_show_last_card(num)
    ...
    local str_pic=string.format("res/cards/MDCards/card_%s%s.png",coler-1,num)
     local spr=display.newSprite(str_pic)
     spr:setPosition(pai:getPositionX(),pai:getPositionY())
     spr:setVisible(false)
    ...
    if round>0 then
      ...
      if self.delegate.type~=0 then
        ...
        self:visibleWxyq(true)
         ...
         -- self:is_start_but_touch(false)
          self:visibleWxyq2(true)
          local paytype = LocalData:Instance():get_RoomInfo().param_g

          if paytype == 3 or paytype == 4 then
             self.gameScene:getChildByName("rmoveroom"):setVisible(true)
          else
             self.gameScene:getChildByName("bt_tc"):loadTextures("niuniu_youxijiemian/n_bt_jiesanfangjian.png","niuniu_youxijiemian/n_bt_jiesanfangjian1.png")
          end
...
function gameControl:visibleWxyq(vis)
   if getYKLogin() then
      vis = false
   end
   print("dddd ",vis)
   printError("那儿设置的啊啊啊啊啊啊")
   self.gameScene:getChildByName("n_star_node"):getChildByName("bt_yq_wx_1"):setVisible(vis)
end


function gameControl:visibleWxyq2(vis)
   if getYKLogin() then
      vis = false
   end
   -- print("dddd ",vis)
   -- printError("那儿设置的啊啊啊啊啊啊")
   self.gameScene:getChildByName("n_star_node"):getChildByName("bt_yq_wx_2"):setVisible(vis)
end
...
--   N_USER_STATE_READY,   //玩家准备完毕，等待抢庄;
--   N_USER_STATE_ZHUANG,  //玩家抢庄完毕，等待下注;
--   N_USER_STATE_BET,     //玩家下注完毕，等待配牛;
--   N_USER_STATE_OPEN,    //玩家配牛完毕，等待结算;
--   N_USER_STATE_WAIT_JION,    //玩家进入房间等待这一局结束;
--   N_USER_STATE_END,    
-- };

function gameControl:players_Info(dex,user_info,zhuangpos,roomInfo, roomdata)
    print("玩家w位置  = ", dex)
    print("庄的位置 = ",zhuangpos)
    print("房间数据 = ",roomdata)
    print("房间信息 = ",roomInfo)
    print("玩家信息 = ",user_info)
    ...
    self:ref_all_head(user_info.head,dex)
      ...
      -- 等待玩家抢庄
    if (user_info.userstate == 1 and roomdata.state == 3) then
         self.user_Banker_buf[1][dex]:setVisible(true)
         print("等待玩家抢庄 = ",user_info)
          self:function_Desk_Z(true) 
         -- self.user_Banker_buf[1][dex]:setContentSize(cc.size(240, 96))
         self.user_Banker_buf[1][dex]:loadTexture("niuniu_youxijiemian/n_frame_qiangzhuangzhong.png") 
      else
         -- self.user_Banker_buf[1][dex]:setContentSize(cc.size(127, 60))
      end
      
    -- self:user_state_upd()
    if (user_info.userstate == 1 and roomInfo~=1 and roomdata.state ~= 3) or (user_info.userstate==2 and roomInfo==1) then --1准备
        self.user_Banker_buf[1][dex]:setVisible(true)
        self.user_Banker_buf[1][dex]:loadTexture("niuniu_youxijiemian/n_ing_zhunbei.png")
        print(dex,"等待玩家1准备 = ",user_info)
        if dex==6 then
           self:button_control(2)
        end
        -- self:is_start_but_touch(true)
      ...
      if user_info.userstate==3 then --4下注
           print(dex,"等待玩家1下注 = ",user_info)
        self.user_Banker_buf[1][dex]:setVisible(false)
        self.user_Banker_buf[2][dex]:setVisible(true)
        self.user_Banker_buf[2][dex]:loadTexture("niuniu_youxijiemian/n_ing_xiazhubeishu"..user_info.betvalue..".png")
      end
...
function gameControl:roomInfo_control(strcu)
  ...
  -- roomInfo.rule_c ---抵住，控制倍数按钮
    print("等待玩家下注 =  ",ZTControl.hasintogame,roomInfo.param_c,strcu)
  
   if roomInfo.param_c == 1 and ZTControl.hasintogame then --基础倍数
      self.delegate.gameScene:getChildByName("bt_db_type_1"):setVisible(strcu)
    end 

    if roomInfo.param_c==2 and ZTControl.hasintogame then --5倍数
       self.delegate.gameScene:getChildByName("bt_db_type_2"):setVisible(strcu)
    end 

    if roomInfo.param_c==3 and ZTControl.hasintogame then --10倍数
...
function gameControl:back_online_card()
      ...
      if not ZTControl.hasintogame then
         self.user_all_num = self.user_all_num - 1
      end

      print(ZTControl.hasintogame,"当前玩家的总数是多少 = ",self.user_all_num)
      for i=1,self.user_all_num do
        action_Offset=i
        local table_board={}
        
        if i == self.user_all_num and ZTControl.hasintogame then
           sub_pos=150
           action_Offset=6
        end
        ...
        for j=1,5 do
          if action_Offset == 6 then
             self.mode_board_obj[#self.mode_board_obj]:setScale(1)
...
function gameControl:break_online(msgtype,ifniu)
    print("强壮阶段的牌信息  = ", msgtype,ifniu)
    ...
    if msgtype == 3 or  msgtype == 4 or msgtype == 5 or msgtype == 6 then
       self:self_CardMsg()
       print("显示自己的牌")
    else
      print("显示别人的的牌")
...
function gameControl:room_status_tip(actionType,type)
    print("类型  = ",type)
    printError("房间提示语----"..actionType)
    if type==1 then
      -- Tools.showError("房间进入提示语 =  "..actionType)
    ...
    if actionType==N_DESK_STATE.N_DESK_STATE_BET  then --房间进入下注
           local msg=LocalData:Instance():get_UserActionMsgRes()
           if msg  then --and msg.actionType ~=4
              self.tip_status:getParent():stopAllActions()
              self.tip_status:getParent():setVisible(false)
            end
            -- self.tip_status:loadTexture("niuniu_youxijiemian/n_ing_dengdaixiazhu.png")
          end

         if actionType==N_DESK_STATE.N_DESK_STATE_OPEN then --房间进入比牛
          self.tip_status:getParent():stopAllActions()
          self.tip_status:getParent():setVisible(false)
          -- self.tip_status:loadTexture("niuniu_youxijiemian/n_ing_dengdaopeiniu.png")
         end
...
function gameControl:UserJoinRoomMsgRes()
     ...
     -- if c_msg.userstate == 5 then
     --    setZTControl(false)
     -- end
     local roomInfo=c_msg.roomInfo
     local userInfo=c_msg.users.pos
     print("玩家加入房间时候的状态  = ",json.encode(decodeMsgData(c_msg)))
     
     local intodata = decodeMsgData(c_msg)
     -- 判断当前玩家是否处于等待状态
     local waitstatue = false
     -- if #intodata.users > 2 then
     --     local curusertatue = intodata.users[#intodata.users].userstate
     --     for i,user in ipairs(intodata.users) do
     --         if user.userstate ~= curusertatue and i < #intodata.users and user.userstate > 1 and curusertatue > 1  then
     --            waitstatue = true
     --            break
     --         end
     --     end
     ...
     if c_msg.users[#c_msg.users].userstate == 5 then
        waitstatue = true
        setZTControl(false)
     else
        setZTControl(true)
     ...
     if AllRequire.ui.GameScene.GameScene then
        AllRequire.ui.GameScene.GameScene:showReady(not waitstatue)
        print("是否处于等待状态1222  = ",waitstatue)
     end

     print("是否处于等待状态  = ",waitstatue)

     local self_num=6
     self.user_all_num_new=1
     self:user_room_id(roomInfo.roomID,roomInfo.round,roomInfo.param_a,roomInfo.state)
     -- 判断当前的玩家的状态 和是否 已经加入房间
        ...
        for i=1,#self.user_buf do
            if i==self_num and c_msg.users[j].pos== c_msg.m_pos then
                local user_info=c_msg.users[j]
                  self:players_Info(i,user_info,c_msg.zhuangpos,c_msg.roomInfo.param_b,c_msg.roomInfo)
            end
...
function gameControl:NotifyRoomUserCardMsg()
   local c_msg=LocalData:Instance():get_NotifyRoomUserCardMsg()
    print("------两排信息下---",json.encode(decodeMsgData(c_msg)))
   dump(c_msg.usercount)
    ...
    print(ZTControl.hasintogame,"self.delegate.gameScene:visibleRemoveRoomButton ",self.delegate.gameScene:getChildByName("rmoveroom"))
    if c_msg.round and c_msg.round == 1 then
       self.delegate.gameScene:getChildByName("rmoveroom"):setVisible(false)
    end
    if c_msg.userCard and type(c_msg.userCard) == "table" and #c_msg.userCard > 0 then
    ...
    for i=1,#self.user_buf-1 do
      if tonumber(self.user_buf[i].pos)== tonumber(c_msg.actionPos) then
        -- print("准备打印了没----",c_msg.actionType,c_msg.actionPos,c_msg.zhuangPos)
          -- Tools.showError("房间状态更新   "..c_msg.actionType)
          self:user_state_upd(i,c_msg)

         if self.user_buf[i] then
            if c_msg.userstate and c_msg.userstate == 5 then
               self.user_buf[i].user_c:getChildByName("wait"):setVisible(true)
            else
               self.user_buf[i].user_c:getChildByName("wait"):setVisible(false)
            end
          end
...
function gameControl:NotifyRoomUserMsg()
  ...
  -- dump(msg.msg_ID)
  -- dump(msg.usercount)
   print("当前社么意思啊啊啊 = ",json.encode(decodeMsgData(msg)))
    local c_msg= msg.users[1]
    
    for i,usmsg in ipairs(msg.users) do
        if self.user_buf[usmsg.pos] then
            if usmsg.userstate == 5 then
               print("self.user_buf[i].user_c:getChildByName", self.user_buf[usmsg.pos].user_c:getChildByName("wait"))
               self.user_buf[usmsg.pos].user_c:getChildByName("wait"):setVisible(true)
            else
               self.user_buf[usmsg.pos].user_c:getChildByName("wait"):setVisible(false)
            end
        end
    end
...
function gameControl:once_Settlement()
      ...
      print("当前的状态ZTControl.hasintogame = ",ZTControl.hasintogame)
      if not ZTControl.hasintogame then 
         self.countTime:close_update()
         self:function_Desk_show_Control("Desk_In") 
         return 
      end
        ...
        for j=1,#self.user_buf do
           if self.user_buf[j].pos==msg.result[i].pos then
                ...
                if tonumber(msg.result[i].score) < 0   then
                  ...
                  num_fh:loadTexture("niuniu_gameover/n_ing_jianhao.png")
                else
                  num:setVisible(true)
                  num_L:setVisible(false)
                  num_fh:loadTexture("niuniu_gameover/n_ing_jiahao.png")
...
function gameControl:onEnter()
      ...
      NotificationCenter:Instance():AddObserver("NotifyRoomUserCardMsg", self,--手牌消息
                       function()
                                ...
                                print("----------手牌消息---")
                                local c_msg=LocalData:Instance():get_NotifyRoomUserCardMsg()
                                if c_msg.roomstate > 2 and ZTControl.hasintogame then 
                                  self.gameScene:getChildByName("bt_tc"):setVisible(false)
                                end
