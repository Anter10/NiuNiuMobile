...
function voiceLayer:fun_voiceAct()
          ...
          voiceAct:setPosition(cc.p(display.cx,display.cy))
          ...
          voiceAct_act:gotoFrameAndPlay(0,60, true)

          if not self.ttlayer then
              self.ttlayer = cc.LayerColor:create(cc.c4f(39,40,34,124))
              self.ttlayer:setAnchorPoint(cc.p(0.5, 0.5))
             
              self.ttlayer:setContentSize(cc.size(300,300))
              self:addChild(self.ttlayer,0,29998)
              -- voiceAct:setScale(0.8)
              self.ttlayer:setPosition(display.cx - 180, display.cy-140)
          end
end

--录音动画存放位置
function voiceLayer:startRecording()
      if self.ttlayer then
         self.ttlayer:setVisible(true)
      end  
