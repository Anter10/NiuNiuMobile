...
function chatLayer:fun_act_img(_obj,start_img,_oneself)
  dump(start_img)
    ...
    cache:addSpriteFrames("gameover/GameChatexpression.plist", "gameover/GameChatexpression.png")
    local m_pSprite1 = cc.Sprite:create("gameover/touming.png" )
    if _oneself then
       self._oneself=1
    end
...
function chatLayer:onExit()
    local cache = cc.SpriteFrameCache:getInstance()
    cache:removeSpriteFramesFromFile("gameover/GameChatexpression.plist")
