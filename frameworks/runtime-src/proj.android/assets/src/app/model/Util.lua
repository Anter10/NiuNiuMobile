...
function Util:share(type_)
   ...
   print("分享提示------",path,content,title)
   local share=cc.UM_Share:createWithShare(path,title,content,openid)
...
function Util:getWeixinLoginDate()
  --is_weixin 字段，-1 代表未进行授权之前状态，0 代表授权成功，1 代表授权失败或者取消授权
          ...
          if tonumber(os.time())-tonumber(curr_time)>7200 then--7200
...
function Util:fun_Offlinepopwindow( popup_text ,_obj,_call)
                    ...
                    _obj:addChild(self.Offlinepopwindow,300,300)
...
function Util:fun_loadbar(_obj,_tag )
  print("旋转动画")
        cc.Director:getInstance():getRunningScene():removeChildByTag(999999, true)
          ...
          _obj:addChild(self.loadbar,_tag,_tag)
...
function Util:fun_notOpen(_obj,_x,_y,_tag)
  local m_pSprite1 = cc.Sprite:create("gameover/m_frame_wenzikuang.png" )
